<template><div><h1 id="technical-debt-and-refactoring" tabindex="-1"><a class="header-anchor" href="#technical-debt-and-refactoring" aria-hidden="true">#</a> Technical debt and refactoring</h1>
<p>No team can entirely avoid technical debt.
To some degree, you will build up technical debt.
There are many reasons for technical debt to occur:</p>
<p>Time pressure, not knowing any better solution, underestimating the impact, not enough communication, priority on new features, ...</p>
<p>It's important to not lose track of it and keep adding features on top of your technical flaws, otherwise you might end up in the feature trap.</p>
<p><img src="@source/inside-agile-product-development-series/09-technical-debt-refactoring/debt.png" alt="Image taken from scrum.org"></p>
<p>Take a look at the image above (taken from <a href="https://www.scrum.org/resources/blog/technical-debt-product-owners" target="_blank" rel="noopener noreferrer">scrum.org<ExternalLinkIcon/></a>) - it demonstrates the trap many teams fall into.
...
I know, building fancy new features is more fun, but being in that feature trap is no fun at all, believe me.</p>
<p>In our business model, we tried out many things and kept adjusting.
Requirements were changing <strong>rapidly</strong>.
...
Taking more time to think about the process, continuously monitor and active alert business issues (on top of technical ones).</p>
<p>The extremely short feedback loop allowed us to quickly iterate features, re-adjust and so on.
Up until initial feature completion, we did very well.
One thing that we really lacked was the continuous monitoring of the features.</p>
<p>Whenever we had some technical debt we wanted to fix, the product managers trusted the developers' assessment.
If the developers said it is urgent or should be fixed soon, they trusted us and we were able to tackle the issue.</p>
<p>As we worked with Scrumban, we didn't reserve a specific amount of time for refactorings or bug fixes.
...
If you pile up bugs or technical debt and ignore them for a little too long, they will haunt you and slow you down so much, you're almost not moving at all.</p>
<p>For a while, whenever a bug occurred, we only fixed that specific occurrence, usually a consequence of old processes that lacked a data cleanup and proper investigation.
...
Do not underestimate that impact.</p>
<p>Another technical flaw that we ignored for a long time was our delta import/export mechanism.
...
That took some time.
Probably a lot more time compared to respecting that criterion from the very beginning or tackling the issue earlier.</p>
<p>As we had a continuous deployment to the production environment, meaning every commit to the main branch landed in production (given the tests and test deployment were successful), every change had to be in a non-breaking way.
...
Besides that, we ran many dry-runs on test and production data to make sure we don't mess up.</p>
<p>New architectural changes to tackle technical debt were also done in a non-breaking way, starting in a single service and figuring out the best way to do it.
Later on, that change was applied to all other services and eventually any remains of the technical debt were removed.</p>
</div></template>
